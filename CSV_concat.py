...
def concatenate_csvs_in_subfolders(root_folder):
    ...
    for subdir, _, files in os.walk(root_folder):
        ...
        # If there are CSV files in the current subdirectory, proceed
        if any(file.endswith('.csv') for file in files):
            ...
            # Create the 'outputs' subdirectory only in the deepest level where files are saved
            output_subdir = os.path.join(runs_folder, relative_path, 'outputs')
            ...
            # Save concatenated DataFrames back to CSV in the corresponding 'runs/outputs' subdirectory
